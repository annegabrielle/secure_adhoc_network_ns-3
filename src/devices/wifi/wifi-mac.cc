...
TypeId
MacTimestampTag::GetTypeId(void)
{
  static TypeId tid = TypeId ("MacTimestampTag")
    .SetParent<Tag> ()
    .AddConstructor<MacTimestampTag> ()
    .AddAttribute ("Timestamp",
                   "Some momentous point in time!",
                   EmptyAttributeValue(),
                   MakeTimeAccessor(&MacTimestampTag::GetTimestamp),
                   MakeTimeChecker())
    ;
  return tid;
}
TypeId
MacTimestampTag::GetInstanceTypeId(void) const
{
  return GetTypeId ();
}

uint32_t
MacTimestampTag::GetSerializedSize (void) const
{
  return 8;
}
void
MacTimestampTag::Serialize (TagBuffer i) const
{
  int64_t t = m_timestamp.GetNanoSeconds();
  i.Write((const uint8_t *)&t, 8);
}
void
MacTimestampTag::Deserialize (TagBuffer i)
{
  int64_t t;
  i.Read((uint8_t *)&t, 8);
...
void
MacTimestampTag::SetTimestamp(Time time)
{
  m_timestamp = time;
}
Time
MacTimestampTag::GetTimestamp(void) const
{
  return m_timestamp;
}

void
MacTimestampTag::Print(std::ostream &os) const
{
  os << "t=" << m_timestamp;
}
