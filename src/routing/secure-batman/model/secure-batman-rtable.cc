...
NS_LOG_COMPONENT_DEFINE ("SecureBatmanRoutingTable");
	 ...
	 m_lastTimeToLive(lastTimeToLive)
{
	...
	m_current_iv = (unsigned char*)malloc(16);
	m_current_key = (unsigned char*)malloc(16);
	m_current_nonce = (unsigned char*)malloc(16);
    m_key_count = 0;
    m_auth_seq_num = 0;
...
void
NeighborInformationEntry::GenerateNeighborKeyStream()
{
	int rand_len = RAND_LEN;
//	m_auth_value = (unsigned char*)malloc(rand_len*10+10);
	m_auth_value_len = 0;
...
//		tool_dump_memory(m_current_nonce, RAND_LEN);
...
uint8_t
OriginatorListEntry::CountRealPackets(SecureBatmanHeader batmanPacket, Ipv4Address sender)
{
	...
	for (std::map<Ipv4Address, NeighborInformationEntry>::iterator i = m_neighborInformation.begin (); i != m_neighborInformation.end (); ++i )
	{
		if ( !isDuplicate )
		{
			isDuplicate = sec_get_bit_status(i->second.real_bits, GetCurrentSeqNumber(), batmanPacket.GetSequenceNumber());
		}

		if (i->first == sender)
		{
			sec_bit_get_packet( i->second.real_bits, batmanPacket.GetSequenceNumber() - GetCurrentSeqNumber(), 1 );
		}
		else
		{
			sec_bit_get_packet( i->second.real_bits, batmanPacket.GetSequenceNumber() - GetCurrentSeqNumber(), 0 );
		}

		i->second.realPacketCount = sec_bit_packet_count( i->second.real_bits );
	}
...
void
OriginatorListEntry::UpdateNeighborRanking(Ipv4Address sender, SecureBatmanHeader batmanPacket, std::map<Ipv4Address, NeighborInformationEntry> &directNeighbors, uint8_t isDuplicate)
{
	...
	for (std::map<Ipv4Address, NeighborInformationEntry>::iterator i = m_neighborInformation.begin (); i != m_neighborInformation.end (); ++i )
	{
		...
		{
			sec_ring_buffer_set(i->second.m_recvTq, &(i->second.m_index), GetNewestTqValue());
			i->second.SetTqAvg(sec_ring_buffer_avg(i->second.m_recvTq));
			neighbor = i->second;
		...
		else
		{
			if(!isDuplicate)
			{
				sec_ring_buffer_set(i->second.m_recvTq, &(i->second.m_index), 0);
				i->second.SetTqAvg(sec_ring_buffer_avg(i->second.m_recvTq));
			}
...
bool
OriginatorListEntry::GetNeighbor2(Ipv4Address sender, Time currentTime, SecureBatmanHeader batmanPacket, NeighborInformationEntry **neighborEntry)
{
...
uint8_t
OriginatorListEntry::BidirectionalCheck(Ipv4Address sender, Time currentTime, SecureBatmanHeader batmanPacket, uint8_t rebroadcastCount, uint8_t tqRecv)
{
...
/* Does not create new neighbor if not found in list */
bool
OriginatorList::GetDirectNeighbor2(Ipv4Address sender, NeighborInformationEntry **neighborEntry)
{
		  if (m_directAuthenticatedNeighbors.empty ())
		    {
		      return false;
		    }
		  std::map<Ipv4Address, NeighborInformationEntry>::iterator i = m_directAuthenticatedNeighbors.find (sender);
		  if (i == m_directAuthenticatedNeighbors.end ())
		    {
		      return false;
		    }
		  *neighborEntry = &(i->second);
...
/* Creates new neighbor if not found in list */
bool
OriginatorList::GetDirectNeighbor(Ipv4Address sender, NeighborInformationEntry **directNeighborEntry)
{
	if (!(m_directAuthenticatedNeighbors.empty()))
	{
		std::map<Ipv4Address, NeighborInformationEntry>::iterator i = m_directAuthenticatedNeighbors.find (sender);
		...
		if (! (i == m_directAuthenticatedNeighbors.end()))
		{
			*directNeighborEntry = &(i->second);
		}
		else
		{
			NS_LOG_DEBUG( sender << " not found in Neighborlist, create new neighbor in DirectAuhenticatedNeighborList.\n");
			NeighborInformationEntry newDirectNeighbor (
					/*neighbor address=*/ sender,
					/*lastValidTime=*/ Simulator::Now(),
					/*lastTTL=*/ 0 );
			std::pair<std::map<Ipv4Address, NeighborInformationEntry>::iterator, bool> result = m_directAuthenticatedNeighbors.insert (std::make_pair (sender, newDirectNeighbor));
			*directNeighborEntry = &(result.first->second);
		}
	}
	else
	{
		NS_LOG_DEBUG("Neighborlist empty, create new neighbor in DirectAuhenticatedNeighborList.\n");
		NeighborInformationEntry newDirectNeighbor (
				/*neighbor address=*/ sender,
				/*lastValidTime=*/ Simulator::Now(),
				/*lastTTL=*/ 0 );
...
bool
OriginatorList::CheckOneTimePassword(Ipv4Address sender, unsigned char *oneTimePassword, uint16_t seqNo)
{
	/* Check OneTimePassword received with OneTimePassword from self-generated key stream */
	NeighborInformationEntry *neighbor = NULL;
	GetDirectNeighbor2(sender, &neighbor);

	if( neighbor == NULL)
	{
		NS_LOG_DEBUG("Neighbor " << sender << " not found in Authenticated List!");
		return false;
	}

	Time timeout = Seconds(60.0);
	if( (Simulator::Now()) > ((neighbor->GetLastValidTime())+timeout) )
	{
		NS_LOG_DEBUG("Neighbor " << sender << "s key stream has timed-out!");
		return false;
	}

	/* Verify One Time Password against Authenticated Key Stream */
	if ( memcmp(neighbor->m_auth_value+seqNo*2, oneTimePassword, 2) == 0 )
	{
		return true;
	}
...
OriginatorList::BitGetPacketNeighbors()
{
	...
	{
		old = i->second.broadcastOwn = sec_bit_packet_count(i->second.bcast_own);
		sec_bit_get_packet( i->second.bcast_own, 1, 0 );
		i->second.broadcastOwn = sec_bit_packet_count(i->second.bcast_own);
//		NS_LOG_DEBUG("Schedule own packet! old = " << (uint16_t)old << " new = " << (uint16_t)i->second.broadcastOwn);
	}
}

void
OriginatorList::UpdateOriginator(Ipv4Address originator, Ipv4Address sender, SecureBatmanHeader batmanPacket, uint8_t isDuplicate)
{
...
OriginatorList::CountRebroadcast (Ipv4Address sender)
{
	...
	if (!(m_directNeighbors.empty()))
	{
		...
		if (! (i == m_directNeighbors.end()))
		{
			...
			sec_bit_mark(i->second.bcast_own, 0);
			i->second.broadcastOwn = sec_bit_packet_count(i->second.bcast_own);
		...
		else
		{
			...
			sec_bit_mark(result.first->second.bcast_own, 0);
			result.first->second.broadcastOwn = sec_bit_packet_count(result.first->second.bcast_own);
...
bool
OriginatorList::GetOriginator2(Ipv4Address originatorAddr, Time currentTime, SecureBatmanHeader batmanPacket, Ptr<NetDevice> dev, Ipv4InterfaceAddress interface,
