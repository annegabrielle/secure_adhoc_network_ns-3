...
NS_LOG_COMPONENT_DEFINE ("BatmanManetExample");

class BatmanManetExample
{
public:
  BatmanManetExample ();
...
int main (int argc, char **argv)
{
  ...
  std::string CSVfileName = "BatmanManetExample.csv";
  ...
  cmd.AddValue ("CSVfileName", "The name of the CSV output file name[Default:BatmanManetExample.csv]", CSVfileName);
  cmd.Parse (argc, argv);
...
BatmanManetExample::BatmanManetExample ()
  : bytesTotal (0),
    packetsReceived (0)
{
}
...
void
BatmanManetExample::ReceivePacket (Ptr <Socket> socket)
{
...
void
BatmanManetExample::CheckThroughput ()
{
  ...
  out.close ();
  packetsReceived = 0;
  Simulator::Schedule (Seconds (1.0), &BatmanManetExample::CheckThroughput, this);
}

Ptr <Socket>
BatmanManetExample::SetupPacketReceive (Ipv4Address addr, Ptr <Node> node)
{
  ...
  sink->Bind (local);
  sink->SetRecvCallback (MakeCallback ( &BatmanManetExample::ReceivePacket, this));

  return sink;
}

void
BatmanManetExample::CaseRun (uint32_t nWifis, uint32_t nSinks, double totalTime, std::string rate,
                           ...
                           double dataStart, bool printRoutes, std::string CSVfileName)
{
  ...
  std::string tr_name = "Batman_Manet_" + t_nodes + "Nodes_" + sTotalTime + "SimTime";
...
void
BatmanManetExample::CreateNodes ()
{
...
void
BatmanManetExample::SetupMobility ()
{
...
void
BatmanManetExample::CreateDevices (std::string tr_name)
{
...
void
BatmanManetExample::InstallInternetStack (std::string tr_name)
{
  BatmanHelper batman;
  batman.Set ("PeriodicUpdateInterval", TimeValue (Seconds (m_periodicUpdateInterval)));
  batman.Set ("SettlingTime", TimeValue (Seconds (m_settlingTime)));
  ...
  if (m_printRoutes)
    {
      ...
      batman.PrintRoutingTableAllAt (Seconds (m_periodicUpdateInterval), routingStream);
    }
}

void
BatmanManetExample::InstallApplications ()
{
