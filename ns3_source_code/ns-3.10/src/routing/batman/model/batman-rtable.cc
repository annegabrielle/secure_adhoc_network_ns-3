...
uint8_t
OriginatorListEntry::DuplicatePacketCheck(BatmanHeader batmanPacket, Ipv4Address sender)
{
	uint8_t isDuplicate = 0;
	uint16_t old = 0;

	for (std::map<Ipv4Address, NeighborInformationEntry>::iterator i = m_neighborInformationList.begin (); i != m_neighborInformationList.end (); ++i )
	{
		if ( !isDuplicate )
		{
...
{
	  std::pair<std::map<Ipv4Address, NeighborInformationEntry>::iterator, bool> result = m_neighborInformationList.insert (std::make_pair (newNeighbor.GetNeighborAddress(),newNeighbor));
	  if (result.second)
	  {
...
{
	if ( !(m_neighborInformationList.empty()) )
	{
		std::map<Ipv4Address, NeighborInformationEntry>::iterator i = m_neighborInformationList.find (sender);

		if( i != m_neighborInformationList.end() )
		{
		...
		else
		{
			...
			std::pair<std::map<Ipv4Address, NeighborInformationEntry>::iterator, bool> result = m_neighborInformationList.insert (std::make_pair (sender, newNeighbor));
...
OriginatorListEntry::PurgeNeighborInformation()
{
	...
	for (std::map<Ipv4Address, NeighborInformationEntry>::iterator i = m_neighborInformationList.begin (); i != m_neighborInformationList.end (); ++i)
	{
		if (Simulator::Now() > (purgeTimeout + i->second.GetLastValidTime()))
		{
			...
			m_neighborInformationList.erase(itmp);
...
{
	m_neighborInformationList.clear();
}
