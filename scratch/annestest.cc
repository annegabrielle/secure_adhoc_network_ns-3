...
void
RoutingPacketTxTrace (std::string context, Ptr<const Packet> p)
{
	rTx++;
//      std::cout << " TX p: " << *p << std::endl;
}
...
void
RoutingPacketReTxTrace (std::string context, Ptr<const Packet> p)
{
	rReTx++;
//      std::cout << " TX p: " << *p << std::endl;
}
...
void
RoutingPacketRxTrace (std::string context, Ptr<const Packet> p)
{
	rRx++;
...
main (int argc, char *argv[])
{
	...
	/* Trace packet sent by OnOff application. UDP packets from source to destination. No BATMAN packets */
	...
	Config::Connect ("/NodeList/*/$ns3::batman::RoutingProtocol/Rx", MakeCallback (&RoutingPacketRxTrace));
	/* Trace routing packets  */
	Config::Connect ("/NodeList/*/$ns3::batman::RoutingProtocol/Tx", MakeCallback (&RoutingPacketTxTrace));
	Config::Connect ("/NodeList/*/$ns3::batman::RoutingProtocol/ReTx", MakeCallback (&RoutingPacketReTxTrace));
	...
	Ptr<PacketCounterCalculator> routingTx = CreateObject<PacketCounterCalculator>();
	routingTx->SetKey("routingTx");
	Config::Connect ("/NodeList/*/$ns3::batman::RoutingProtocol/Tx", MakeCallback(&PacketCounterCalculator::PacketUpdate, routingTx));
	data.AddDataCalculator(routingTx);
	...
	std::cout << "Flow monitor # Packets Tx:\t" << totalPacketsTx << "\n";
	std::cout << "Flow monitor # Packets Rx:\t" << totalPacketsRx << "\n";
	std::cout << "Flow monitor Avg Delay:\t\t" << totalDelay.GetSeconds() << "/" << totalPacketsRx << " = " << (totalDelay.GetSeconds())/totalPacketsRx << "\n\n";

	std::cout << "Trace Onoff packet delay:\t" << t_delay.GetSeconds()  << "/" << n << " = "<< (t_delay.GetSeconds())/n <<"\n\n";
